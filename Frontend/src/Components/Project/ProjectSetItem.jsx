                ...
                <section className='ProjectItem-body'>
                    <div className='Text-box'>
                        <NavLink onClick={onNavigateToProfile} style={{ textDecoration: 'none' }} to='/profile'  >
                            <h3 className='admin-box'>Admin: {pr.project_owner}</h3>
                        </NavLink>
                        {keycloak.hasRealmRole(ROLES.User) && <div style={{ fontWeight: 'bold', color: "#0C0C1E", background: `${checkColor(compatibility())}` }} className="project-compatibility">{compatibility()}</div>}
