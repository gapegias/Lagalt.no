...
const HomePage = () => {
    ...
    // Fetching all needed data from the API and saves them to the session storage
    const getAllData = async () => {
        ...
        let sortedProjects = projectsResponse.map(pr => pr)
        .sort(function (x, y) { return y.project_id - x.project_id })
    ...
    const readUser = async () => {
        if (typeof keycloak.tokenParsed !== 'undefined') {
            ...
            storageSave(STORAGE_KEY_USER_PROFILE, userResponse)                  
            setUser(userResponse)
            setUserProfile(userResponse)
        }
    }

    // Deletes user's data from the session storage after logout
    const deleteUser = ()=>{
